...
function parseJs(code, define) {//js模板解析
    ...
    var promise = {
            ...
            getState: function() { //是否为true
                for (var i = 0, l = this.len - 1; i < l; i++) {
            ...
            push: function(state) {
                this.len++;
                return this.data.push(state), state;
            }
        };

    return function() {

        var res = code.replace(/(\{\{.*?\}\})(((?!\{\{)[\s\S])*)/g, function(phrase, condition, codeSniffer) {//暂不支持自定义openTag和closeTag
            ...
            if (_promise == 'true') return codeSniffer;
